...
USB_UART_COMMAND(ddr2ReadSweepCommand, "DDR2 Read Sweep",
        "Sweeps the DDR2 read-return delay (NXTDATAVDLY) and prints the burst read-back for each, to find the value that reads a full burst correctly") {
...
USB_UART_COMMAND(ddr2SCLSweepCommand, "DDR2 SCL Sweep",
        "Sweeps the DDR2 PHY SCL latency (DDRSCLLAT) live (no SCL re-run) and prints a burst read-back grid (destructive to first 16 bytes of DDR2)") {
...
USB_UART_COMMAND(ddr2XferProbeCommand, "DDR2 Xfer Probe",
        "Probes DDR2 structural transfer-config knobs live (MAXBURST, undocumented XFERCFG<30:28> and MEMCFG0<29>) and prints a burst read-back grid (destructive to first 16 bytes of DDR2)") {
...
USB_UART_COMMAND(ddr2BL4SwapCommand, "DDR2 BL4 Swap",
        "Reprograms the DDR2 DRAM mode register from BL8 to BL4 at runtime and probes a burst read-back (reboot restores BL8; do not run if anything is using DDR2)") {

    (void) input_str;   // no arguments

    terminalTextAttributesReset();
    ddr2SwapBurstLength4();
    terminalTextAttributesReset();

}

USB_UART_COMMAND(ddr2ArbProbeCommand, "DDR2 Arb Probe",
        "Live-tests the DDR2 target-agent arbiter (reference config vs power-on) and auto-precharge on/off, probing a burst read-back at each point (destructive to first 16 bytes of DDR2)") {
...
USB_UART_COMMAND(ddr2AccessProbeCommand, "DDR2 Access Probe",
        "Probes DDR2 reads by access path: CPU 8/16/32-bit reads and a DMA read of the same 16 bytes, to localize the read fault to the CPU bus path vs the DDR controller (destructive to first 16 bytes of DDR2)") {
