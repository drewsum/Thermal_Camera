/*******************************************************************************
  ...
  Description:
    ...
    Clock: SDHCCAP.BASECLK reports the actual base clock frequency (in MHz,
    per SDHCI convention) baked into this silicon instance -- no PBCLK/
    REFCLK is wired to SDHC anywhere else in this codebase, and the ATDF
    exposes no clock-source-select bitfield for it, so the internal clock
    generator's source is fixed in hardware. SDHC_SetClockDivider() reads
    that field back and computes SDCLKDIV from it rather than trusting any
    hardcoded assumption -- this project has already been burned once by an
    incorrect *assumed* clock relationship (the DDR2 driver's MPLL-vs-DRAM-
    clock 2x error), so the SDHC clock math is deliberately self-measuring
...
// (application/power_saving.c), enables the internal clock and waits for
// ICLKSTABLE, runs a full software reset (SWRALL), sets the card-
...
// Reprograms SDHCCON2.SDCLKDIV to produce an SDCLK at or below targetHz,
// using SDHC_GetBaseClockHz() (not a hardcoded assumption) as the
// reference. Gates SDCLKEN off before reprogramming and back on after,
// per the SDHCI spec's required sequencing, waiting ICLKSTABLE both times.
...
// the command. Has no effect on commands issued with dataPresent=false.
...
// uncached (KSEG1) region, points SDHCAADDR at its physical address, and
// waits TXCIF with a bounded timeout, checking SDHCAESTAT on failure.
// Caller must have already issued the command via SDHC_SendCommand(...,
// dataPresent=true) with SDHC_ConfigureBlockTransfer(..., useADMA2=true)
// beforehand (which sets SDHCMODE.DMAEN and SDHCCON1.DMASEL for this
// path). Gated by SDHC_IsADMA2Supported() -- sd_card.c falls back to
// SDHC_TransferBlocksPIO() transparently if this silicon instance doesn't
// support it.
