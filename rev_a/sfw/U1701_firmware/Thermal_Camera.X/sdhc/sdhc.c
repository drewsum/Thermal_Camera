...
bool SDHC_SetClockDivider(uint32_t targetHz)
{
    ...
    // This implementation's 8-bit SDCLKDIV field is the SDHCI Ver2.00-
    // style one-hot-or-zero divisor select (not Ver3.00's 10-bit
    // programmable divider): SDCLK = BaseClock / (2*N) for N != 0, or
    // BaseClock / 1 (bypass) for N == 0. Valid N values are 0 and powers
    // of two up to 0x80 (max divisor 256). Pick the smallest divisor that
    // keeps SDCLK at or below targetHz. NOTE: if the measured base clock
    // is high enough that even a /256 divisor exceeds targetHz (e.g. the
    // 400kHz identification rate), this implementation cannot reach spec
    // -- verify SDHC_GetBaseClockHz() against this ceiling during bring-up
    // (see the implementation plan's Verification section).
    uint16_t n = 0;
    uint32_t divisor = 1;
    while (((sdhc_base_clock_hz / divisor) > targetHz) && (n < 0x80u))
    {
        n = (n == 0) ? 1u : (uint16_t)(n << 1);
        divisor = 2u * n;
    }
    ...
    SDHCCON2bits.SDCLKEN = 0;
    SDHCCON2bits.SDCLKDIV = (uint8_t)n;
...
void SDHC_PrintStatus(void)
{
    ...
    printf("    SD Clock: %s, SDCLKDIV: 0x%02X\n\r",
            SDHCCON2bits.SDCLKEN ? "enabled" : "disabled", (unsigned int)SDHCCON2bits.SDCLKDIV);
    printf("    Base Clock (measured, SDHCCAP.BASECLK): %lu Hz\n\r", (unsigned long)sdhc_base_clock_hz);

    if (SDHCCON2bits.SDCLKEN && (SDHCCON2bits.SDCLKDIV > 0))
    {
        printf("    Calculated SDCLK: %lu Hz\n\r",
                (unsigned long)(sdhc_base_clock_hz / (2u * SDHCCON2bits.SDCLKDIV)));
    }
