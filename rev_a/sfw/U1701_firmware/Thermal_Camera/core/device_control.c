...
// this function sets up peripheral bus clock 6
void PBCLK6Initialize(void);
...
// this function sets up reference clock 4
// REFCLK4 is the SDHC module's base clock on the PIC32MZ DA family (the
// SDHCCAP.BASECLK capability field assumes it runs at 200 MHz). Source and
// divider must be programmed while ON = 0.
void REFCLK4Initialize(void) {

    // Set REFCLK4 divider to 0 (undivided, pass source straight through)
    REFO4CONbits.RODIV = 0;

    // source for REFCLK4 is SYSCLK (200 MHz)
    REFO4CONbits.ROSEL = 0b0000;

    // Keep REFCLK4 running in Idle mode so an idle CPU can't stall an
    // in-flight SDHC transfer
    REFO4CONbits.SIDL = 0;
...
void PBCLK6Initialize(void) {
    ...
    // Set PBCLK6 divider to 3
    PB6DIVbits.PBDIV = 0b0000010;
...
void printClockStatus(uint32_t input_sysclk) {
    ...
    if (REFO1CONbits.ON == 0) {
        ...
        printf("\n\r    REFCLK1 (Reference Clock 1) is set to: %s\n\r",
                stringFromClockSetting(REFO1CONbits.RODIV ? input_sysclk / (2 * REFO1CONbits.RODIV) : input_sysclk));
    ...
    if (REFO2CONbits.ON == 0) {
        ...
        printf("    REFCLK2 (Reference Clock 2) is set to: %s\n\r",
                stringFromClockSetting(REFO2CONbits.RODIV ? input_sysclk / (2 * REFO2CONbits.RODIV) : input_sysclk));
    ...
    if (REFO3CONbits.ON == 0) {
        ...
        printf("    REFCLK3 (Reference Clock 3) is set to: %s\n\r",
                stringFromClockSetting(REFO3CONbits.RODIV ? input_sysclk / (2 * REFO3CONbits.RODIV) : input_sysclk));
    ...
    if (REFO4CONbits.ON == 0) {
        ...
        printf("    REFCLK4 (Reference Clock 4) is set to: %s\n\r",
                stringFromClockSetting(REFO4CONbits.RODIV ? input_sysclk / (2 * REFO4CONbits.RODIV) : input_sysclk));
    ...
    if (REFO5CONbits.ON == 0) {
        ...
        printf("    REFCLK5 (Reference Clock 5) is set to: %s\n\r",
                stringFromClockSetting(REFO5CONbits.RODIV ? input_sysclk / (2 * REFO5CONbits.RODIV) : input_sysclk));
    ...
    if (PB6DIVbits.ON) {
        terminalTextAttributes(GREEN_COLOR, BLACK_COLOR, NORMAL_FONT);
        printf("    PBCLK6 (Peripheral Bus Clock 6) is set to: %s\n\r",
            stringFromClockSetting(input_sysclk / (PB6DIVbits.PBDIV + 1)));
    }
    else {
        terminalTextAttributes(RED_COLOR, BLACK_COLOR, NORMAL_FONT);
        printf("    PBCLK6 (Peripheral Bus Clock 6) is disabled\r\n");
    }
