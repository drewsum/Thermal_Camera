...
/  Thermal_Camera: enabled -- cheap to include now, useful for a future
/  "format SD card" USB UART command. */
...
/  Thermal_Camera: enabled -- sd_fileio.c's volume-info query (backing the
/  "SD Card Info?" command) reports the label via f_getlabel(). */
...
/* Thermal_Camera: US/ASCII -- avoids pulling in ffunicode.c's DBCS/LFN
/  conversion tables, which aren't vendored (see FF_USE_LFN below). */
...
#define FF_USE_LFN		0
/* Thermal_Camera: 8.3 short names only. Image/text filenames written by
/  this firmware (e.g. IMG00001.BMP) fit 8.3 fine, and disabling LFN means
/  ffunicode.c (15k+ lines of DBCS/Unicode conversion tables) doesn't need
/  to be vendored at all. Revisit if descriptive long filenames become a
/  real requirement. */
