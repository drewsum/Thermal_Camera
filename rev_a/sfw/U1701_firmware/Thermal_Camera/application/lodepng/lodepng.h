...
/* ---------------------------------------------------------------------------
   LOCAL MODIFICATION (Thermal_Camera firmware) -- per license term 2, this
   vendored copy is altered: the block below is the only change, selecting
   lodepng's own documented compile options (each is honored via the
   #ifndef guards further down in this unmodified header):

   - Decoder only (no PNG encoder -- this firmware never writes PNGs)
   - No stdio disk I/O (files are read via FatFs by
     application/image_loader.c, which hands lodepng a memory buffer)
   - No built-in malloc/realloc/free: image_loader.c provides
     lodepng_malloc()/lodepng_realloc()/lodepng_free() backed by a DDR2
     arena, because a decoded frame + zlib working buffers exceed this
     device's 115KB internal heap
   - No ancillary chunk parsing (text/time/etc. metadata is unused)
   - Error text strings kept, for useful terminal diagnostics
--------------------------------------------------------------------------- */
