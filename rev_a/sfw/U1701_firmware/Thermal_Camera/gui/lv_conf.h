...
/*
 * Thermal Camera (U1701) configuration for the vendored LVGL in gui/lvgl.
 * This is a copy of gui/lvgl/lv_conf_template.h with the deltas listed
 * below; everything else is left at its upstream default so this file can
 * be re-diffed against the template on an LVGL version bump.
 *
 * It is found via LV_CONF_INCLUDE_SIMPLE plus "gui" on the include path,
 * both set in cmake/Thermal_Camera/default/user.cmake.
 *
 *   LV_COLOR_DEPTH        32       ARGB8888, matching GLCD Layer 1's color
 *                                  mode (glcd.h) so LVGL renders straight
 *                                  into the overlay with no conversion.
 *   LV_MEM_SIZE           4MB      LVGL's heap, which also backs lodepng's
 *   LV_MEM_ADR            DDR2+3MB allocations (application/image_loader.c
 *                                  decodes PNGs through it). Cached KSEG0
 *                                  alias -- CPU-only memory, the GLCD's DMA
 *                                  never reads it. See gui/gui.h for the
 *                                  full DDR2 partition map.
 *   LV_DRAW_BUF_ALIGN     16       Cache-line friendly for DDR2 bursts.
 *   LV_USE_LOG            1        Routed to the USB UART terminal and the
 *   LV_LOG_USE_TIMESTAMP  0        error handler by gui.c (no lv_tick-based
 *                                  timestamps -- the terminal has its own).
 *   LV_ASSERT_HANDLER     GUI_...  Latch an error flag and keep running
 *                                  instead of upstream's while(1) -- the GUI
 *                                  is not critical, but the UART console,
...
#define LV_STDARG_INCLUDE       <stdarg.h>
    ...
    #define LV_MEM_SIZE (4 * 1024 * 1024U)    /**< [bytes] -- DDR2, see gui/gui.h */

    /** Size of the memory expand for `lv_malloc()` in bytes */
    #define LV_MEM_POOL_EXPAND_SIZE 0

    /** Set an address for the memory pool instead of allocating it as a normal array. Can be in external SRAM too. */
    #define LV_MEM_ADR 0x88300000     /**< DDR2 +3MB, cached KSEG0 alias -- see gui/gui.h */
...
#define LV_USE_DRAW_SW 1
     ...
     */
    /* THERMAL_CAMERA: these gate the blend-into-<format> code paths by
     * DESTINATION buffer format, and every buffer this target renders into
     * is ARGB8888 (GLCD Layer 1, see glcd.h) -- so all the others are dead
     * weight worth about 120KB of the 2MB program flash. Kept on:
     *   ARGB8888  the overlay buffers themselves
     *   A8        the format LVGL uses for mask/alpha layers, which is how
     *             font glyphs get drawn
     *   RGB888    per the note above, gradients render through it
     * If something ever renders blank rather than wrong, re-enable the
     * format its destination buffer actually uses before looking further. */
...
#define LV_ASSERT_HANDLER_INCLUDE "gui/gui_assert.h"
#define LV_ASSERT_HANDLER GUI_AssertHandler();   /**< Latch an error flag and keep running */
...
/* THERMAL_CAMERA: lv_font_montserrat_14_aligned.c is a stride-aligned
 * duplicate of the 14px font that upstream defaults to ON inside the source
 * file itself, so without this it lands in the image (~30KB) even though
