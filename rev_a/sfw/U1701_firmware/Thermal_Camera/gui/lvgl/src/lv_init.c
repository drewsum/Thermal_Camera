...
/* LOCAL MODIFICATION (Thermal_Camera): the includes for libs/bmp, ffmpeg,
 * freetype, gif, tjpgd, libjpeg_turbo, libpng and tiny_ttf were removed here
 * because those directories were deleted from this vendored tree (see
 * src/drivers/lv_drivers.h for why the tree is trimmed rather than merely
 * configured off). Every call into them below is already inside a
 * #if LV_USE_<LIB> block and all of those are 0 in gui/lv_conf.h, so nothing
 * else needs to change. Restore both the directories and these includes if
 * one of those decoders is ever wanted. */
